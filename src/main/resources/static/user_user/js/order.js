            ...
            switch (data.code){
                ...
                case "1111":
                    for (var i in  data.data){
                        ...
                        var endDate = dateFormat(data.data[i].orderEndTime);
                        var endTime = TimeFormat(data.data[i].orderEndTime);
                        htmlStr+= "<div class=\"ticketCon\" onclick=\"viewOrder(\'"+data.data[i].orderCard+"\',\'"+data.data[i].orderId+"\')\">\n" +
                            "        <div class=\"timeCon\">\n" +
                            "          <div class=\"ticketTime\">\n" +
                            "            <div>出发时间</div>\n" +
                            "            <div class=\"dateItem\">"+startDate+"</div>\n" +
...
function viewOrder(carCard,orderId){
    // location.href=("/user/toOrderDetails");
    $.ajax({
        url: window.location.protocol+"//"+window.location.host+"/user/doCommOrder",
        type:"POST",
        async:false,
        data:JSON.stringify({"carCard":carCard,"orderId":orderId}),
        contentType:"application/json;charset=UTF-8",
        success:function(data){
        },
        error:function(){
