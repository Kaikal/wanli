...
function minus(btnId) {
	var num = document.getElementById(btnId);
	if (num.value>=1){
		num.value=parseInt(num.value)-1;
	}
}
function userTipBtn(){   //num传递的是session中的用户名
	location.href=("index");
}
function getTickNum(){
	$.ajax({
		url: window.location.protocol+"//"+window.location.host+"/user/getTickNum",
		type:"POST",
		async:false,
...
function addOrder(carMess,VipNum,OrdinaryNum,NoSiteNum,startTime,endTime){
	let param = {
		"carId":carMess.carId,
		"carCard":carMess.carCard,
		"carRoute":carMess.carRoute,
		"vip":VipNum,
		"ordinary":OrdinaryNum,
		"nos":NoSiteNum,
		"startTime":startTime,
		"endTime":endTime
	}
	$.ajax({
		url: window.location.protocol+"//"+window.location.host+"/user/addOrder",
		type:"POST",
		async:false,
		...
		success:function (data){
			if (data.code=="0000"){
				...
				location.href=("about?goals=viewOrder**carId**"+carMess.carId+"**orderId**"+data.data);
			}
		},
		error:function(){
			alert("请求错误了");
...
function getTimeDifference(){
	...
	if (startTime!=undefined&&startTime!=null&&startTime!=''&&endTime!=undefined&&endTime!=null&&endTime!=''){
		time = timeDifference(d1,d2);
		$('#timeDifference').val(time);
	}
}
